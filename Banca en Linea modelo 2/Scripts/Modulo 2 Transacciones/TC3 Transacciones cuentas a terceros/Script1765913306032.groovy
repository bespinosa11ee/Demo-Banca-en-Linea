...
if (WebUI.verifyElementPresent(transferenciaTercero, 5, FailureHandling.OPTIONAL)) {
    WebUI.click(transferenciaTercero)
...
// Cerrar sesion
TestObject buttonLogout = findTestObject('Object Repository/Modulo 1/Login/boton logout')

// Verificamos si el botón está presente en la página
if (WebUI.verifyElementPresent(buttonLogout, 5, FailureHandling.OPTIONAL)) {
	WebUI.click(buttonLogout)

	println('Se hizo clic el boton cerrar sesion correctamente')
} else {
	println('No se pudo hacer clic en el boton cerrar sesion')
}
